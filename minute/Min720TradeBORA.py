...
def get_target_price(ticker, k):
    ...
    df = pyupbit.get_ohlcv(ticker, interval="minute=720", count=2)
    target_price = df.iloc[0]['close'] + (df.iloc[0]['high'] - df.iloc[0]['low']) * k
    return target_price

def get_start_time(ticker):
    """시작 시간 조회"""
    df = pyupbit.get_ohlcv(ticker, interval="minute=720", count=1)
...
while True:
    try:
        ...
        end_time = start_time + datetime.timedelta(minutes=720)
